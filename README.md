...
- полностью плоская главная схема: **никаких вложенных subcircuit-блоков**;
- только базовые компоненты Logisim Evolution и реальные электрические линии;
...
3. Наблюдайте за четырьмя ROM, верхней магистралью, регистрами всех контекстов,
   АЛУ, управляющей ROM и общей RAM. Когда программа закончится, `AllHalted`
   станет единицей.
...
Дополнительная структурная проверка разбирает XML проекта: каждый компонент
главного листа обязан принадлежать стандартной библиотеке Logisim. Поэтому
вычислитель нельзя случайно снова спрятать во вложенную схему.
